document.addEventListener('DOMContentLoaded', () => {
    ...
    const digitalClockSkinSubmenu = document.getElementById('digitalClockSkinSubmenu'); // デジタル用
    const analogClockSkinSubmenu = document.getElementById('analogClockSkinSubmenu'); // アナログ用
    const digitalClockSizeSubmenu = document.getElementById('digitalClockSizeSubmenu'); // デジタル用
    const analogClockSizeSubmenu = document.getElementById('analogClockSizeSubmenu'); // アナログ用
    ...
    // 時計サイズ選択肢のイベントリスナー（これはHTMLに直接記述）
    // PC/モバイル表示切り替えのサブメニューと共通のhasSubmenuItemsで処理される

    // ページロード時にスキンとサイズ設定を読み込む
    const savedDigitalSkinIndex = localStorage.getItem('digitalClockSkinIndex');
    const savedAnalogSkinIndex = localStorage.getItem('analogClockSkinIndex');
    const savedDigitalSize = localStorage.getItem('digitalClockSize');
    const savedAnalogSize = localStorage.getItem('analogClockSize');
    const savedClockType = localStorage.getItem('clockType');

    if (savedDigitalSkinIndex !== null) currentDigitalSkinIndex = parseInt(savedDigitalSkinIndex);
    if (savedAnalogSkinIndex !== null) currentAnalogSkinIndex = parseInt(savedAnalogSkinIndex);
    if (savedDigitalSize) currentDigitalSize = savedDigitalSize;
    if (savedAnalogSize) currentAnalogSize = savedAnalogSize;
    if (savedClockType) currentClockType = savedClockType;
    ...
    sidebar.addEventListener('click', (event) => {
        ...
        const size = targetLink.dataset.size; // サイズ変更用
        const clockType = targetLink.dataset.clockType; // 時計タイプ変更用
